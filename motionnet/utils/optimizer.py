...
class Optimizer(object):
    def __init__(self, net_paramters, config):
        self.config = config
        ...
        # optimizer
        if config["opt"] == "sgd":
            self.opt = torch.optim.SGD(net_paramters,
                                       lr=config['init_lr']*lr_scale,
                                       weight_decay=config['weight_decay'])
        elif config["opt"] == "adam":
            self.opt = torch.optim.Adam(net_paramters,
                                        lr=config['init_lr']*lr_scale,
                                        weight_decay=config['weight_decay'])
        elif config["opt"] == "adamw":
            self.opt = torch.optim.AdamW(net_paramters,
                                         lr=config['init_lr']*lr_scale,
                                         weight_decay=config['weight_decay'])
        else:
